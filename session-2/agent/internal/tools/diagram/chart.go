...
// exChartElements renders a chart for Excalidraw. Bars and pie legends become
// rectangles, a line series becomes a line element — all editable primitives
// rather than a flattened picture.
//
// Pie slices are the one thing that doesn't survive the trip: Excalidraw has no
// arc primitive, so the scene gets the legend and a labelled circle instead of
// wedges. The SVG is the faithful rendering for pies.
func exChartElements(n *diagramNode, nextID func(string) string) []map[string]any {
	var out []map[string]any
	top := n.y
	if n.label != "" {
		out = append(out, exText(nextID("charttitle"), n.label, n.x, n.y, 18, "left", "", ""))
		top += chartTitleGap
	}
	baseline := top + chartPlotHeight
	max := chartMax(n.data)
	...
	case "pie":
		total := chartTotal(n.data)
		id := nextID("pie")
		out = append(out, exBase(id, "ellipse", n.x, top, 2*pieRadius, 2*pieRadius, exSeed(id)))
		for i, d := range n.data {
			ly := top + float64(i)*24
			swatch := nextID("swatch")
			sw := exBase(swatch, "rectangle", n.x+2*pieRadius+16, ly, 14, 14, exSeed(swatch))
			sw["backgroundColor"] = exFillFor(i)
			out = append(out, sw)
			out = append(out, exText(nextID("legend"),
				fmt.Sprintf("%s — %.0f%%", d.Label, 100*d.Value/total),
				n.x+2*pieRadius+38, ly, 14, "left", "", ""))
		}
	...
	case "line":
		...
		id := nextID("line")
		el := exBase(id, "line", n.x, top, n.w, chartPlotHeight, exSeed(id))
		el["points"] = pts
		el["lastCommittedPoint"] = nil
		out = append(out, el)
		for i, d := range n.data {
			out = append(out, exText(nextID("cat"), d.Label,
				n.x+pts[i][0]-20, baseline+8, 14, "center", "", ""))
		}
	...
	default: // bar
		...
		for i, d := range n.data {
			h := chartPlotHeight * (d.Value / max)
			id := nextID("bar")
			bar := exBase(id, "rectangle", x, baseline-h, chartBarWidth, h, exSeed(id))
			bar["backgroundColor"] = exFillFor(i)
			out = append(out, bar)
			out = append(out, exText(nextID("cat"),
				fmt.Sprintf("%s (%s)", d.Label, formatValue(d.Value)),
				x, baseline+8, 14, "center", "", ""))
			x += chartBarWidth + chartBarGap
		}
	}
	return out
