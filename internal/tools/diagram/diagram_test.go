...
func TestGenerateDiagramRedrawOverwrites(t *testing.T) {
	...
	// Exactly the four files the tool owns — the SVG, the converted scene, the
	// skeletons it was converted from, and the spec the CRUD tools edit — with
	// no accumulating per-draw clutter.
	entries, _ := os.ReadDir(dir)
	got := map[string]bool{}
	for _, e := range entries {
		got[e.Name()] = true
	}
	if len(entries) != 4 || !got[canvasFile] || !got[excalidrawFile] || !got[specFile] || !got[skeletonFile] {
		t.Fatalf("expected exactly %s, %s, %s and %s, got %v",
			canvasFile, excalidrawFile, specFile, skeletonFile, entries)
	}
}
