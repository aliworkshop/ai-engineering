...
> **Branch `session-2`.** Each session of the course is a branch, with the agent
> at the repo root — `git switch session-2` then `go run .`. `session-1` is the
> same agent without the diagram tools; `session-3` renders diagrams with
> Excalidraw itself.
